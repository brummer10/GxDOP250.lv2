...
G_DEFINE_TYPE(GxDOP250PaintBox, gx_paint_box, GTK_TYPE_BOX)

static void gx_paint_box_class_init (GxDOP250PaintBoxClass *klass)
{
...
static void set_paint_func(GxDOP250PaintBox *paint_box, const gchar *paint_func)
{
	gchar *spf;
...
static void gx_paint_box_style_set(GtkWidget *widget, GtkStyle  *previous_style)
{
	GxDOP250PaintBox *paint_box = GX_DOP250_PAINT_BOX(widget);
	set_paint_func(paint_box, paint_box->paint_func);
}


static void gx_paint_box_init (GxDOP250PaintBox *paint_box)
{
...
static void gx_paint_box_destroy(GtkObject *object)
{
	GxDOP250PaintBox *paint_box = GX_DOP250_PAINT_BOX(object);
	...
	GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	switch (prop_id) {
	case PROP_PAINT_FUNC:
		g_value_set_string(value, GX_DOP250_PAINT_BOX(object)->paint_func);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
		break;
...
static void pedal_expose(GtkWidget *wi, GdkEventExpose *ev)
{
	...
	cairo_t *cr;
	GxDOP250PaintBox *paintbox = GX_DOP250_PAINT_BOX(wi);
