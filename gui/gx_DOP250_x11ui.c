...
static void resize_event(gx_DOP250UI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_DOP250UI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	ui->controls[0] = (gx_controller) {{ 1.0, 1.0, 0.0, 1.0, 1.0}, { 40, 40, 61, 61}, false,"POWER", BSWITCH, BYPASS};
	ui->controls[1] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 120, 40, 61, 61}, false,"GAIN", KNOB, GAIN};
	ui->controls[2] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 200, 40, 61, 61}, false,"LEVEL", KNOB, LEVEL};
	ui->block_event = -1;
...
static void knob_expose(gx_DOP250UI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_DOP250UI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_DOP250UI *ui, gx_controller* controller) {
...
static void _expose(gx_DOP250UI *ui) {
	static const char* plug_name = "GxDOP250" ;
	...
	cairo_text_extents_t extents;
	cairo_set_source_rgba (ui->cr, 0.1, 0.1, 0.1, 1.0);
	...
	cairo_move_to (ui->cr, ((double)(ui->width/2.0)/ui->rescale.x-(extents.width)/2.0),
	  (double)(ui->height-35.0)/ui->rescale.y-extents.height);
...
static void controller_expose(gx_DOP250UI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_DOP250UI *ui, int controller) {
...
static void check_value_changed(gx_DOP250UI *ui, int i, float* value) {
...
static void check_is_active(gx_DOP250UI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_DOP250UI *ui) {
...
static bool get_active_ctl_num(gx_DOP250UI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_DOP250UI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_DOP250UI *ui, int direction) {
	float value;
	int num;
...
static void switch_event(gx_DOP250UI *ui, int i) {
...
static void button1_event(gx_DOP250UI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_DOP250UI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_DOP250UI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_DOP250UI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_DOP250UI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_DOP250UI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_DOP250UI *ui, bool set) {
...
static void event_handler(gx_DOP250UI *ui) {
	XEvent xev;
