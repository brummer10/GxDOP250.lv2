...
bool gx_gui_open_display(gx_DOP250UI *ui) {
	return true; // STUB
}

void gx_gui_create_window_and_surface(gx_DOP250UI *ui) {
	// prepare window class
	static TCHAR szClassName[] = TEXT("gx_DOP250UIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_register_controller_message(gx_DOP250UI *ui) {
	// dummy, not required on MSWin
}

void gx_gui_destroy_main_window(gx_DOP250UI *ui) {
	DestroyWindow(ui->win);
	// safe to use: doesnt unregister if there are still windows of this class
	UnregisterClass(TEXT("gx_DOP250UIClass"), NULL);
}

void gx_gui_resize_surface(gx_DOP250UI *ui) {
	RECT rect;
...
void gx_gui_send_controller_event(gx_DOP250UI *ui, int controller) {
...
void event_handler(gx_DOP250UI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_DOP250UI *ui = (gx_DOP250UI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
